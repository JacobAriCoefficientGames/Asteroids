...
public class Play extends BasicGameState{
	...
	public void render(GameContainer gc, StateBasedGame sbg, Graphics g)throws SlickException {
		...
		g.drawString( "Score: "+score, SetupClass.ScreenWidth,SetupClass.ScreenHeight/100); //draw Score
		
		if(levelTicks<5)
			g.drawString( "Score: "+score, SetupClass.ScreenWidth,SetupClass.ScreenHeight/100); //draw Level number if it is a new level
	}
		...
		//RENDER GAME OVER TODO
		private void renderGameOver(GameContainer gc, StateBasedGame sbg, Graphics g) {
			g.drawString("Game Over",SetupClass.ScreenWidth/2,SetupClass.ScreenHeight/2);
		}
	...
	public void update(GameContainer gc, StateBasedGame sbg, int delta)
		...
		addPendingAsteroids();
		if(isLevelClear())makeNewLevel();
		updateAsteroids(gc, sbg, delta);
		updateShip(gc, sbg, delta);
		}
		if(SetupClass.isDEBUGGING)updateCounter++;
	}
		...
		if(in.isKeyPressed(Input.KEY_P)){isPaused=!isPaused;return;}		//pause the game when player presses 'P'
		if(in.isKeyPressed(Input.KEY_SPACE))ship.shoot();			//fire ship guns when player presses Spacebar
		if(in.isKeyDown(Input.KEY_W))ship.ForwardKeyPressed();
		if(in.isKeyDown(Input.KEY_A))ship.LeftKeyPressed();
		if(in.isKeyDown(Input.KEY_D))ship.RightKeyPressed();
		if(in.isKeyDown(Input.KEY_S))ship.BackKeyPressed();
	}
